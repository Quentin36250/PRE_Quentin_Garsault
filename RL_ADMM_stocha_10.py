...
class ADMM_Env_rho_unique(Env):
    def __init__(self):
        ...
        self.observation_space=Box(low=1E-6,high=1,shape=(20,))
        self.state=[1 for i in range(0,20)]
        self.length=1
    ...
    def step(self,action):
        ...
        self.state.append((primal_residual))
        self.state.append((test*dual_residual))
    ...
    def reset(self):
        ...
        for j in G.nodes():
            G.nodes[j]['P_cons']= mem_p_con[j]*(1+random.randint(-10,10)/100)
            G.nodes[j]['Q_cons']= mem_q_con[j]*(1+random.randint(-10,10)/100)
            G.nodes[j]['S_max']= mem_p_max[j]*(1+random.randint(-10,10)/100)
        ...
        for ind in range(10):
            ...
            self.state[2*ind]=(primal_residual)
            self.state[2*ind+1]=(dual_residual)
        self.length=10
        return self.state
